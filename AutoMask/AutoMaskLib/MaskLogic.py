...
#
# Param:       inputImage: The input greyscale image to be masked
#              outputImage: The output image to store the mask
#              sigma
#              lowerThreshold
#              upperThreshold
...
class MaskLogic:
    """This class provides methods for automatic masking"""
    ...
    def fillHole(self, img, foreground):
        ...
        for i in range(depth):
            im = img[:,:,i]
            mask = fill_hole_filter.Execute(im)
            vectorOfImages.push_back(mask)
    ...
    def erode(self, img, radius, foreground):
        ...
        mask_img = erode_filter.Execute(img)
    ...
    def automask_ormir(
        ...
        auto_mask = AutocontourKnee()
        masks = []

        # Find mask for each bone
        for i in range(boneNum):
            mask = auto_mask.get_periosteal_mask(img, i+1)
            masks.append(mask)
    ...
    def execute(self, step, alg):
        ...
        if alg == 0:
            self.closeBone()
            self.masks = self.automask_ormir(self.model_img, self.boneNum)
